...
def main(image_dir, output_predictions):
    
    pretrained_model = "SenseTime/deformable-detr"
    model = DeformableDetrForObjectDetection.from_pretrained(pretrained_model)
    processor = DeformableDetrImageProcessor.from_pretrained(pretrained_model)
    model.eval()
...
if __name__ == "__main__":
    ...
    # sys.argv[2] is ignored, but accepted for compatibility
    output_predictions = sys.argv[3]
    main(image_dir, output_predictions)
